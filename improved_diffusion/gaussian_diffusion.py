...
class GaussianDiffusion:
    ...
    :param betas: 一个一维 numpy 数组，表示每个扩散时间步的 beta 值，
                  从 T（最大时间步）到 1 递减。
    :param model_mean_type: 指定模型输出的均值类型。
    :param model_var_type: 指定模型输出的方差类型。
    :param loss_type: 指定训练中使用的损失函数类型。
    :param rescale_timesteps: 如果为 True，则将时间步重新缩放到一个标准化范围（0 到 1000）。
    :param model_arch: 指定模型的架构，例如 'conv-unet' 等。
    :param training_mode: 设置训练模式，例如 'emb' 表示基于嵌入的训练模式，'e2e' 表示端到端训练。
    """

    def __init__(
        self,
        *,
        betas,  # 扩散过程的 beta 值数组。
        model_mean_type,  # 均值预测类型，例如 'eps'、'x_start'、'v'。
        model_var_type,     # 方差预测类型，例如 'fixed_small'、'fixed_large'。
        loss_type,  # 损失函数类型，例如 'L1'、'L2'、'VLB'。
        rescale_timesteps=False,  # 是否重新缩放时间步，作为模型输入。
        model_arch=None,   # 模型架构（可选参数）。
        training_mode='emb',  # 默认训练模式为基于嵌入（embedding-based）。
        ...
        # 将 betas 转换为高精度（float64）的 numpy 数组。
        betas = np.array(betas, dtype=np.float64)
        self.betas = betas

        # 检查 betas 是否为一维数组，且所有值在 (0, 1] 范围内。
        assert len(betas.shape) == 1, "betas must be 1-D"
        assert (betas > 0).all() and (betas <= 1).all()
        # 根据 betas 的长度确定扩散过程的总时间步（T）。
        self.num_timesteps = int(betas.shape[0])

        # 计算每个时间步的 alpha（1 - beta）的累积乘积。
        alphas = 1.0 - betas
        self.alphas_cumprod = np.cumprod(alphas, axis=0)
        # 计算前一个时间步的 alpha 累积乘积。
        self.alphas_cumprod_prev = np.append(1.0, self.alphas_cumprod[:-1])
        # 计算后一个时间步的 alpha 累积乘积。
        self.alphas_cumprod_next = np.append(self.alphas_cumprod[1:], 0.0)
        # 确保 alphas_cumprod_prev 的形状与时间步数一致。
        ...
        # 计算后验方差的对数值，并对第一个时间步的值进行裁剪以保证稳定性。
        self.posterior_log_variance_clipped = np.log(
            np.append(self.posterior_variance[1], self.posterior_variance[1:])
        )
        # 计算后验均值系数，用于计算q(x_{t-1} | x_t, x_0)。
    ...
    def p_mean_variance2(
        ...
        if self.model_mean_type == ModelMeanType.PREVIOUS_X:
            ...
            #有修改，之前是x_start=pred_xstart, x_t=x, t=t)
    ...
    def get_x_start(self, x_start_mean, std):
        '''
        使用插值策略还是使用卷积策略…
        ：param x_start_mean: x_start的平均值（期望值）。
        ：param std：生成x_start的标准差。
        ：返回：计算x_start添加噪声。
        '''
        noise = th.randn_like(x_start_mean) # 生成与 x_start_mean 同样形状的标准正态噪声
        # print(std.shape, noise.shape, x_start_mean.shape)
        assert noise.shape == x_start_mean.shape    # 确保噪声的形状与均值形状一致
        # print(x_start_mean.device, noise.device)
        return (
             x_start_mean + std * noise # 用标准差对噪声进行缩放并加到均值上，得到最终的 x_start
        )

    def token_discrete_loss(self, x_t, get_logits,input_ids):
    #x_t：表示经过某个时间步扩散过程后的数据。在这里，x_t 是一个张量，形状为 [batch_size, seq_len, feature_dim]
    #get_logits(reshaped_x_t)：该函数会将 x_t 作为输入，生成对应的 logits。logits 是网络输出的未经过 softmax 归一化的预测值，通常具有形状 [batch_size, seq_len, vocab_size]，其中 vocab_size 表示词汇表的大小。
    #CrossEntropyLoss：交叉熵损失函数用于计算分类问题中预测和实际标签之间的差异。在这里，交叉熵损失用于计算 logits 与 input_ids 之间的差异，input_ids 是实际的标签数据，形状为 [batch_size, seq_len]。
    #logits.view(-1, logits.size(-1)) 和 input_ids.view(-1)：这里将 logits 和 input_ids 展开为一维数组，以便计算交叉熵损失。logits 的形状被转换为 [batch_size * seq_len, vocab_size]，input_ids 被转换为 [batch_size * seq_len]。
    #计算的是模型输出与实际 input_ids 之间的离散的交叉熵损失。具体来说，它通过计算模型预测的每个时间步的 logit 与实际输入 input_ids 的差异来衡量模型的性能。
        if self.model_arch == 'conv-unet' or  self.model_arch == '1d-unet': # 对于卷积网络模型或1D-Unet结构，调整输入的形状
            reshaped_x_t = x_t.view(x_t.size(0), x_t.size(1), -1).permute(0, 2, 1)  # 将x_t的维度调整为 (batch_size, seq_len, feature_dim)
        else:
            # print(x_t.shape)
            reshaped_x_t = x_t  # 如果模型架构不是 'conv-unet' 或 '1d-unet'，则保持 x_t 的形状不变
        # logits = get_logits(reshaped_x_t)  # bsz, seqlen, vocab
        
        logits = get_logits(reshaped_x_t)   # 通过 get_logits 函数得到模型预测的 logits，形状通常是 (batch_size, seq_len, vocab_size)

        loss_fct = th.nn.CrossEntropyLoss(reduction='none') # 使用 CrossEntropyLoss 计算单个样本的负对数似然
        # 将 logits 和 input_ids 展开为一维，计算交叉熵损失后，再将结果重塑为与原始 input_ids 形状一致
        decoder_nll = loss_fct(logits.view(-1, logits.size(-1)), input_ids.view(-1)).view(input_ids.shape)
        decoder_nll = decoder_nll.mean(dim=-1)  # 对每个时间步的损失进行平均，输出最终的损失值
        return decoder_nll

    def x0_helper(self, model_output, x, t):
        # # 如果模型均值类型是 'PREVIOUS_X'，则从当前状态 x_t 预测前一状态 x_t-1。
        if self.model_mean_type == ModelMeanType.PREVIOUS_X:
            pred_xstart =  self._predict_xstart_from_xprev(x_t=x, t=t, xprev=model_output)# 使用 x_t-1（前一个时间步）预测 x_start
            pred_prev = model_output    ## 预测的前一个状态就是模型输出
        # 如果模型均值类型是 'START_X' 或 'EPSILON'，则采用不同的方式计算 x_start。
        elif self.model_mean_type in [ModelMeanType.START_X, ModelMeanType.EPSILON]:
            #如果模型均值类型是 'START_X'，则预测的 x_start 就是模型输出。
            if self.model_mean_type == ModelMeanType.START_X:
                pred_xstart = model_output
            # 如果模型均值类型是 'EPSILON'，则从 epsilon 预测 x_start。
            else:
    ...
    def training_losses_e2e(self, model, micro, t, model_kwargs=None, noise=None):
        """
        计算单个时间步的训练损失。
        :param model: 模型实例，用于计算损失。
        :param micro: 输入数据的元组，包含多个张量：
                      - input_ids: 原始输入序列。
                      - desc_state: 描述符状态。
                      - desc_mask: 描述符掩码。
                      - corrupt_ids: 已损坏的输入序列。
        :param t: 当前批次的时间步张量。
        :param model_kwargs: 传递给模型的额外参数（可选）。
        :param noise: 用于重构的高斯噪声（可选）。
        :return: 包含损失值的字典。
        """
        # 从 micro 中提取输入数据
        input_ids = micro[0]
        ...
        assert(corrupt_ids.shape==input_ids.shape)  # 确保 corrupt_ids 的形状与 input_ids 相同
        # assert((corrupt_ids==input_ids).all()) # only for check


        #########################################
        mix_ids = th.where(t.reshape(-1,1)<400,corrupt_ids,input_ids) # 根据时间步 t 动态选择是否使用 corrupt_ids
        if t.max()>self.maxt:   # 打印接收到的最大时间步 t，用于调试
            self.maxt = t.max()
            print('Recieving max t:{}'.format(self.maxt))
        ##########################################
        # th.tensor([0]).to('cuda')
        # 从模型嵌入层获取 input_ids 和 mix_ids 的嵌入
        ...
        # 使用标准差计算 x_start 和 mix_start 的初始值
        ...
        # 如果未指定噪声，则生成与 mix_start 形状相同的随机噪声
        if noise is None:
            noise = th.randn_like(mix_start)
        # 通过重参数化技巧生成噪声扰动的输入 x_t
        ...
        # 如果损失类型为均方误差或缩放的均方误差
            ...
            target = {
                ...
                #    x_start=x_start, x_t=x_t, t=t
                #)[0],
                ModelMeanType.START_X: x_start, # 初始值
